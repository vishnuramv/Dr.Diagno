...
const SymptomsForm = () => {
  ...
  const handleChange = (value, symptom) => {
    ...
    const index = symptomsCopy.findIndex(
      (currentValue) => currentValue === symptom
    );
  ...
  return (
    <div className="symptomsForm">
      <ArrowBackIcon onClick={() => history.goBack()} className="back__btn" />
      <h1>Fill this out!</h1>
            ...
            (symptom, index) =>
              symptom.value === 1 && (
                <p key={index}>
                  ...
                  <Cancel
                    style={{ color: green[600], cursor: "pointer" }}
                    fontSize="small"
                    onClick={() => unselectSymptom(symptom)}
                  />
                </p>
              )
          )}
