...
export default function Home() {
  ...
  return (
    <div className="home">
      <Header
        imgURL={BASEURL + userInfo.profile_pic}
            ...
            <div className="home__predictionContainer">
              {previousPredictions.map((previousPrediction, index) => {
                const topDisease = getTopDisease(
                  previousPrediction.predictedDisease
                );
                return (
                  <div key={index} className="home__prediction">
                    <div>
                      <h4>
                        {new Date(previousPrediction.date)
                          .toString()
                          .slice(0, 21)}
                      </h4>
                      <p>
                    ...
                    <IconButton
                      onClick={() =>
                        history.push("/symptom-result", {
                          predictedDisease: previousPrediction.predictedDisease,
                        })
                      }
                    >
